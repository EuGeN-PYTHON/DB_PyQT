...
'_PYIBoot_SPLASH'. The bootloader creates a server_pack socket and accepts every connection request. Since the os-module,
...
def _initialize():
    ...
    try:
        ...
        raise ConnectionError("Unable to connect to the tcp server_pack socket on port %d" % _ipc_port) from err
...
try:
    ...
    # log-level: error
    _log(40, "Cannot connect to the bootloaders ipc server_pack socket", exc_info=_err)
...
def close():
    """
    Close the connection to the ipc tcp server_pack socket.
