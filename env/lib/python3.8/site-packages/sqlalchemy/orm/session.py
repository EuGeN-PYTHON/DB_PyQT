...
class Session(_SessionClassMethods):
    ...
    def bulk_save_objects(
        ...
        then server_pack-generated primary key values will be assigned to the
        returned objects, but **not server_pack side defaults**; this is a
    ...
    def bulk_insert_mappings(
        ...
        :param render_nulls: When True, a value of ``None`` will result
         ...
         that server_pack-side defaults which are invoked by an omitted column will
         be skipped, so care must be taken to ensure that these are not
         necessary.

         .. warning::

            When this flag is set, **server_pack side default SQL values will
            not be invoked** for those columns that are inserted as NULL;
            the NULL value will be sent explicitly.   Care must be taken
            to ensure that no server_pack-side default functions need to be
