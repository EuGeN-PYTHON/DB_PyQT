...
configured on the client_pack machine.   The application then specifies the name
...
is configured on the client_pack, a basic DSN-based connection looks like::
...
database using Azure credentials::
    ...
    connection_string = "mssql+pyodbc://@my-server_pack.database.windows.net/myDb?driver=ODBC+Driver+17+for+SQL+Server"
