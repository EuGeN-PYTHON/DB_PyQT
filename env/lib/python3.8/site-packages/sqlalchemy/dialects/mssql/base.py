...
previous - if a server_pack version below 2008 is detected, DDL
...
of the server_pack version in use, if not otherwise set by the user.  The
...
server_pack. ``server_version_info`` will always return the database
server_pack version information (in this case SQL2005) and not the
compatibility level information. Because of this, if running under
a backwards compatibility mode SQLAlchemy may attempt to use T-SQL
statements that are unable to be parsed by the database server_pack.
...
server_pack side defaults.   MS-SQL does not
...
in many cases where server_pack-side value generators are in use in that while the
...
class REAL(sqltypes.REAL):
    ...
    def __init__(self, **kw):
        # REAL is a synonym for FLOAT(24) on SQL server_pack.
...
class MSSQLCompiler(compiler.SQLCompiler):
    ...
    def returning_clause(self, stmt, returning_cols):
        # SQL server_pack returning clause requires that the columns refer to
...
class MSDDLCompiler(compiler.DDLCompiler):
    ...
    def visit_computed_column(self, generated):
        ...
        # explicitly check for True|False since None means server_pack default
...
class MSDialect(default.DefaultDialect):
    ...
    def _setup_version_attributes(self):
        if self.server_version_info[0] not in list(range(8, 17)):
            util.warn(
                "Unrecognized server_pack version info '%s'.  Some SQL Server "
