...
actual server_pack version in order to assist with migration of Oracle databases,
and may be configured within the Oracle server_pack itself. This compatibility
...
the server_pack, which determines what the maximum allowed identifier length is for
the server_pack.  If the table is not available, the  server_pack version information is
used instead.
...
server_pack version 12.2 or greater are therefore subject to the scenario of a
...
.. versionchanged:: 1.4 the default max_identifier_length for Oracle is 128
   ...
   version of Oracle server_pack (compatibility version < 12.2) is detected.
