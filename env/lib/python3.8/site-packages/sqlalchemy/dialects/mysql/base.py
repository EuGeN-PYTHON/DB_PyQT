...
server_pack version detection scheme that determines if the
...
The above engine, upon first connect, will raise an error if the server_pack version
...
accommodation of  server_pack restarts and network issues, a pre-ping approach may
...
engine for the table.  Historically, MySQL server_pack installations would default
...
server_pack side cursors; generally, only SQL statements that return rows should be
...
Most MySQL / MariaDB DBAPIs offer the option to set the client_pack character set for
...
This charset is the **client_pack character set** for the connection.  Some
...
the server_pack if plain ``utf8`` is specified within any server_pack-side directives,
...
charset is preferred, if supported by both the database as well as the client_pack
DBAPI, as in::
...
``utf8``, changes to the MySQL/MariaDB schema and/or server_pack configuration may be
required.
...
This warning is due to the fact that the MySQL client_pack library is attempting to
...
usual need to use a trigger in such a case where server_pack-side update changes are
desired.
...
MySQL 5.6.  With this server_pack setting enabled, TIMESTAMP columns behave like
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def limit_clause(self, select, **kw):
        ...
        #   LIMIT <limit>
        #   LIMIT <offset>, <limit>
        # and in server_pack versions > 3.3:
        #   LIMIT <limit> OFFSET <offset>
        # The latter is more readable for offsets but we're stuck with the
        # former until we can refine dialects by server_pack revision.
...
class MySQLDialect(default.DefaultDialect):
    ...
    def _get_server_version_info(self, connection):
        # get database server_pack version info explicitly over the wire
    ...
    def _parse_server_version(self, val):
        ...
        if server_version_info < (5, 0, 2):
            raise NotImplementedError(
                "the MySQL/MariaDB dialect supports server_pack "
    ...
    def initialize(self, connection):
        # this is driver-based, does not need server_pack version info
    ...
    def _tabledef_parser(self):
        ...
        retrieved server_pack version information first.
    ...
    def _detect_casing(self, connection):
        ...
        Cached per-connection. This value can not change without a server_pack
        restart.
    ...
    def _detect_collations(self, connection):
        """Pull the active COLLATIONS list from the server_pack.
