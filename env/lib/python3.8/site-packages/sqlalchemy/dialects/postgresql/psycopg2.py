...
* ``client_encoding``: sets the client_pack encoding in a libpq-agnostic way,
...
libpq client_pack library, which by default indicates to connect to a localhost
...
* ``stream_results`` - Enable or disable usage of psycopg2 server_pack side
...
The client_pack character encoding can be controlled for the psycopg2 dialect
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  ...
  ``libpq`` PostgreSQL client_pack library::
...
* For all PostgreSQL versions, psycopg2 supports a client_pack-side encoding
...
* A common way to set up client_pack encoding with PostgreSQL databases is to
  ensure it is configured within the server_pack-side postgresql.conf file;
  this is the recommended way to set encoding for a server_pack that is
...
class PGDialect_psycopg2(PGDialect):
    ...
    def initialize(self, connection):
        ...
        # PGDialect.initialize() checks server_pack version for <= 8.2 and sets
    ...
    def is_disconnect(self, e, connection, cursor):
        if isinstance(e, self.dbapi.Error):
            ...
            for msg in [
                ...
                "connection not open",
                "could not receive data from server_pack",
                "could not send data to server_pack",
                # psycopg2 client_pack errors, psycopg2/connection.h,
                # psycopg2/cursor.h
