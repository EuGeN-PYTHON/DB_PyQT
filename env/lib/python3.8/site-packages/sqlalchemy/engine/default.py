...
class DefaultDialect(interfaces.Dialect):
        ...
        if server_side_cursors:
            if not self.supports_server_side_cursors:
                raise exc.ArgumentError(
                    "Dialect %s does not support server_pack side cursors" % self
                )
            else:
    ...
    def _check_max_identifier_length(self, connection):
        """Perform a connection / server_pack version specific check to determine
...
class DefaultExecutionContext(interfaces.ExecutionContext):
    ...
    def _init_compiled(
        ...
        # has to be regexed in some cases for server_pack side cursor
        if util.py2k:
